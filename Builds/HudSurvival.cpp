...
namespace ve::ui
{
	...
	{
		const float centerX = window.GetWidth() / 2.0f;
		const float centerY = window.GetHeight() / 2.0f;
		...
		const float hotbarX = (window.GetWidth() / 2.0f) - (hotbarWidth / 2.0f);
		const float hotbarY = window.GetHeight() - hotbarHeight;
		...
		const float experienceX = (window.GetWidth() / 2.0f) - (experienceWidth / 2.0f);
