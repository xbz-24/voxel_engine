...
namespace ve::rendering
{
	...
	bool VulkanBackend::Initialize(const VulkanBackendSettings& settings, Window& window)
	{
		...
		if (!surface_.Create(context_.Instance(), window)) { Release(); return false; }
		if (!physical_device_.Select(context_.Instance(), surface_.Handle())) { Release(); return false; }
		if (!device_.Create(physical_device_.Handle(), physical_device_.QueueFamilies())) { Release(); return false; }
		if (!swapchain_.Create(physical_device_.Handle(), device_.Handle(), surface_.Handle(), window.GetWidth(), window.GetHeight())) { Release(); return false; }
		return true;
	}
