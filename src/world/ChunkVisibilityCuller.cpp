...
namespace ve::world::visibility
{
	...
	{
		const float min_x = ve::core::ToFloat(chunk_x * terrain::ChunkWidth) - 0.5f;
		const float min_z = ve::core::ToFloat(chunk_z * terrain::ChunkDepth) - 0.5f;
		const glm::vec3 minimum_corner(min_x, -0.5f, min_z);
		const glm::vec3 maximum_corner(
			min_x + ve::core::ToFloat(terrain::ChunkWidth),
			ve::core::ToFloat(terrain::ChunkHeight) - 0.5f,
			min_z + ve::core::ToFloat(terrain::ChunkDepth));
