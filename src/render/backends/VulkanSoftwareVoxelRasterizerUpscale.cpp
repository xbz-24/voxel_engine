...
namespace ve::rendering
{
	namespace
	{
		...
		{
			return ve::core::ToU32((numerator + ve::core::ToU64(denominator) - 1ull) / denominator);
		}
	}
	...
	void VulkanSoftwareVoxelRasterizer::RebuildUpscaleLookup()
	{
		...
		for (std::uint32_t render_pixel_x = 0; render_pixel_x < render_extent_.width; ++render_pixel_x)
		{
			upscale_x_ranges_[render_pixel_x] = {
				CeilDiv(ve::core::ToU64(render_pixel_x) * extent_.width, render_extent_.width),
				CeilDiv(ve::core::ToU64(render_pixel_x + 1u) * extent_.width, render_extent_.width)
			};
		}

		upscale_y_ranges_.resize(render_extent_.height);
		for (std::uint32_t render_pixel_y = 0; render_pixel_y < render_extent_.height; ++render_pixel_y)
		{
			upscale_y_ranges_[render_pixel_y] = {
				CeilDiv(ve::core::ToU64(render_pixel_y) * extent_.height, render_extent_.height),
				CeilDiv(ve::core::ToU64(render_pixel_y + 1u) * extent_.height, render_extent_.height)
			};
		}
	}
	...
	void VulkanSoftwareVoxelRasterizer::UpscaleRenderPixels()
	{
		...
		const std::size_t row_bytes = ve::core::ToIndex(extent_.width) * sizeof(std::uint32_t);
		for (std::uint32_t source_y = 0; source_y < render_extent_.height; ++source_y)
		{
			...
			const std::uint32_t* source_row = render_pixels_.data() + (ve::core::ToIndex(source_y) * render_extent_.width);
			std::uint32_t* first_destination_row = pixels_.data() + (ve::core::ToIndex(y_range.begin) * extent_.width);
			for (std::uint32_t source_x = 0; source_x < render_extent_.width; ++source_x)
			{
				...
				{
					std::fill(
						first_destination_row + x_range.begin,
						first_destination_row + x_range.end,
						source_row[source_x]);
				}
			}
			...
			{
				std::uint32_t* destination_row = pixels_.data() + (ve::core::ToIndex(destination_y) * extent_.width);
