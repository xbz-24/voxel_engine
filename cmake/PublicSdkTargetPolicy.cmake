function(ve_verify_public_sdk_target target)
    ...
    set(definition_targets "${target}" ${actual_internal_targets})
    foreach(internal_target IN LISTS definition_targets)
        foreach(definition_property IN ITEMS COMPILE_DEFINITIONS INTERFACE_COMPILE_DEFINITIONS)
            get_target_property(target_definitions "${internal_target}" "${definition_property}")
            if (target_definitions MATCHES "(^|[^A-Za-z0-9_])(ROOT_DIR|VE_VULKAN_SHADER_DIR)(=|[^A-Za-z0-9_]|$)")
                message(FATAL_ERROR
                    "${internal_target} embeds legacy runtime path definition "
                    "in ${definition_property}: ${target_definitions}")
            endif()
        endforeach()
    endforeach()
